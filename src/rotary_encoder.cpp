...
RotaryEncoder::RotaryEncoder(int gpio, int rotationPulses)
    : _gpio(gpio), _rotationPulses(rotationPulses),
      _buffSize(rotationPulses / 4) {
  ...
  polling_thread = std::thread(&RotaryEncoder::poll_gpio, this, _gpio);
}
...
double RotaryEncoder::rpm(int order = 0) {
  ...
  if (_buffer.size() >= (2 + order)) {
    std::chrono::steady_clock::time_point t_latest =
        _buffer.at(0); // latest timestamp
    std::chrono::steady_clock::time_point t_second_latest =
        _buffer.at(1); // second latest timestamp
    std::chrono::steady_clock::time_point now =
        std::chrono::steady_clock::now();

    if (std::chrono::duration_cast<std::chrono::milliseconds>(now - t_latest)
            .count() < 200) { // check if standing still (last pulse later than
                              // 200ms ago -> 15 RPM)
      double duration =
          double(std::chrono::duration_cast<std::chrono::microseconds>(
                     t_latest - t_second_latest)
                     .count()) /
          (order + 1);
      ...
      return 1000000 * 60 /
             (duration *
              _rotationPulses); // 1000000 because the timestamps are in
...
void RotaryEncoder::poll_gpio(int gpio) {
  ...
  while (true) {
    ...
    if (gpio_read(gpio)) { // on rising edge (read gpio to verify if rising or
